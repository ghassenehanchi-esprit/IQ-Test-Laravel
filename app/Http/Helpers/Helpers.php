...
function calculateTotalQuizScore($quizId)
{
    ...
    foreach ($quiz->quizDetails as $quizDetail)  {
        // Assuming the relationship is 'category' and 'score' is an attribute
        $categoryScore = $quizDetail->question->category->score;
...
function calculateCorrectAnswers($quizId)
{
    // Find the quiz
$quiz = Quizz::find($quizId);
...
foreach ($quiz->quizDetails as $quizDetail) {
    // Check if the user's answer is not null and is correct
    if (!is_null($quizDetail->user_answer) && $quizDetail->user_answer->is_correct) {
