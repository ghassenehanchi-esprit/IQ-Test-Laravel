...
class FrontendController extends Controller {
    ...
    public function importContent(Request $request, $key) {
                ...
                foreach ($toTempJson['content'] as $toContentKey => $toContentValue) {
                    ...
                    if ($toContentKey == 'images') {
                        foreach ($toContentValue as $imageKey => $imageValue) {
                            fileManager()->removeFile($path . '/' . @$toFrontendContent->data_values->$imageKey);
                            fileManager()->removeFile($path . '/thumb_' . @$toFrontendContent->data_values->$imageKey);
        ...
        if (@$toTempJson['element']) {
            ...
            $toTempElements = Frontend::where('template_name', $toTemp)->where('data_keys', $key . '.element')->get()->pluck('data_values');

            if (isset($toTempJson['element']['images'])) {
                $images = array_keys($toTempJson['element']['images']);
                $files = [];
                foreach ($images as $image) {
                    $files[] = $toTempElements->pluck($image)->toArray();
                    $files[] = $toTempElements->pluck($image)->toArray();
                }

                $filesToRemove = array_unique(array_merge(...$files));
            }
            ...
            foreach ($filesToRemove as $fileName) {
                ...
                fileManager()->removeFile($path . '/thumb_' . @$fileName);
            }
                ...
                foreach ($toTempJson['element'] as $toElementKey => $toElementValue) {
                    ...
                    if ($toElementKey == 'images') {
                        foreach ($toElementValue as $imageKey => $imageValue) {
                            $file = $toTempElements->pluck($imageKey);
